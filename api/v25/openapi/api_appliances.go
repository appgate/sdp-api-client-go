...
type ApiAppliancesIdAdminInterfaceCsrPostRequest struct {
	ctx        context.Context
	ApiService *AppliancesApiService
	id         string
}

func (r ApiAppliancesIdAdminInterfaceCsrPostRequest) Execute() (*CertificateSigningRequest, *http.Response, error) {
	return r.ApiService.AppliancesIdAdminInterfaceCsrPostExecute(r)
}

/*
AppliancesIdAdminInterfaceCsrPost Generate a CSR for the Appliance admin interface HTTPS certificate.

Generate a Certificate Signing Request (CSR) for the admin interface HTTPS certificate of an Appliance. The CSR can then be signed by an external Certificate Authority and uploaded back as public certificate.

	@param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
	@param id ID of the object.
	@return ApiAppliancesIdAdminInterfaceCsrPostRequest
*/
func (a *AppliancesApiService) AppliancesIdAdminInterfaceCsrPost(ctx context.Context, id string) ApiAppliancesIdAdminInterfaceCsrPostRequest {
	return ApiAppliancesIdAdminInterfaceCsrPostRequest{
		ApiService: a,
		ctx:        ctx,
		id:         id,
...
func (a *AppliancesApiService) AppliancesIdAdminInterfaceCsrPostExecute(r ApiAppliancesIdAdminInterfaceCsrPostRequest) (*CertificateSigningRequest, *http.Response, error) {
	var (
		localVarHTTPMethod  = http.MethodPost
		localVarPostBody    interface{}
		formFiles           []formFile
	...
	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "AppliancesApiService.AppliancesIdAdminInterfaceCsrPost")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/appliances/{id}/admin-interface/csr"
...
Runs ICMP ping command on an Appliance. This API call must be made with **Accept** header of **application/vnd.appgate.peer-v25+text** as it returns plain text instead of JSON.
...
Runs tcpdump command on an Appliance. This API call must be made with **Accept** header of **application/vnd.appgate.peer-v25+text** as it returns plain text instead of JSON.
